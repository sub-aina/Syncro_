...
import React, { useState } from "react";
import { useDispatch, useSelector } from "react-redux";
import { submitCheckIn, fetchCheckIns } from "../redux/checkinslice"; // Import fetchCheckIns
import { useNavigate } from "react-router-dom";
...
const DailyCheckin = () => {
	...
	const handleSubmit = async (e) => {
		...
		try {
			// Submit the check-in
			await dispatch(submitCheckIn(form)).unwrap();
			...
			// Refresh the check-ins data immediately
			await dispatch(fetchCheckIns());

			alert("Check-in submitted successfully!");
			navigate("/dashboard/checkins");
		} catch (err) {
			console.error("Error submitting check-in:", err);
			// Error handling is already managed by Redux state
		}
	};
